...
const STEPS = [
  { label:"Load & Preprocess CT Scan",         sub:"Grayscale · Normalize · 512×512",          color:"#0d7377" },
  { label:"Estimate Noise σ (Wavelet/MAD)",     sub:"Donoho-Johnstone HH1 sub-band estimator",  color:"#0a5c60" },
  { label:"Anisotropic Gaussian Filter (AGF)",  sub:"G(x,y)=exp(-(x²+y²)/2σ²) · edge-adaptive",color:"#2d8c5c" },
  { label:"Haar Wavelet DWT (Level 2)",         sub:"LL2 + LH,HL,HH sub-bands decomposed",      color:"#0d7377" },
  { label:"BayesShrink Soft Thresholding",      sub:"λ=σ²_noise/σ_signal · per sub-band",       color:"#0a5c60" },
  { label:"DnCNN Inference (17 layers)",        sub:"64 filters · BatchNorm + ReLU · skip conn.",color:"#2d8c5c" },
  { label:"Inverse Haar DWT (IDWT)",            sub:"Reconstruct from thresholded sub-bands",    color:"#0d7377" },
  { label:"Total Variation Smoothing",          sub:"Remove residual artifacts · weight=0.03",   color:"#0a5c60" },
  { label:"Compute PSNR / SSIM / MSE / SNR",   sub:"Full quality metrics evaluation",           color:"#2d8c5c" },
];
...
export default function Denoise() {
  ...
  const runPipeline = async () => {
    ...
    const animDone = new Promise(async (resolve) => {
            ...
            <>
              ...
              <div className="grid grid-cols-4 gap-1 mt-2">
                {[["PSNR",preview.metrics?.psnr?.toFixed(1)+" dB"],
                  ...
                  ["SNR", preview.metrics?.snr?.toFixed(1)+" dB"]].map(([k,v])=>(
            ...
            {STEPS.map((step, i) => {
              ...
              return (
                <div key={i} className={clsx(
                  ...
                  {result?.stage_times && done && (
                    <span className="ml-auto text-[10px] font-mono flex-shrink-0" style={{ color:G }}>
                      {result.stage_times[
                        ["","noise_estimation_ms","agf_ms","wavelet_ms","wavelet_ms",
                         "wavelet_ms","dncnn_ms","dncnn_ms","tv_ms",""][i+1]
                      ] ?? ""}
                    </span>
                  )}
                </div>
        ...
        {result ? (
          <>
            ...
            <div className="card p-4">
              ...
              <a href={`/api/v1/denoise/${result.id}/download`}
            ...
            <div className="card p-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  {k:"psnr", label:"PSNR", unit:" dB",  color:T,    hint:"Higher = better",    pct: Math.min(100,result.metrics?.psnr/36*100)},
                  {k:"ssim", label:"SSIM", unit:"",     color:G,    hint:"1.0 = perfect match", pct: result.metrics?.ssim*100},
                  {k:"mse",  label:"MSE",  unit:"",     color:"#c47d1e",hint:"Lower = better",  pct: Math.max(0,100-result.metrics?.mse/500*100)},
                  {k:"snr",  label:"SNR",  unit:" dB",  color:"#1a6b8a",hint:"Signal-to-noise", pct: Math.min(100,result.metrics?.snr/25*100)},
                ].map(({k,label,unit,color,hint,pct}) => (
                  <div key={k} className="rounded-xl p-3" style={{ background:"var(--c-surface3)", border:`1px solid var(--c-border)` }}>
